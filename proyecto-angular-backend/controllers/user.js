    ...
    User.findOne({ email: user.email.toLowerCase() }, (err, issetUser) => {
      if (err) {
        ...
        if (!issetUser) {
          ...
          bcrypt.hash(params.password, null, null, function (err, hash) {
            ...
            user.save((err, userStored) => {
              if (err) {
                res
                  .status(500)
                  .send({ menssage: "Error al guardar el usuario" });
              } else {
                if (!userStored) {
                  res
                    .status(404)
                    .send({ menssage: "No se ha registrado el usuario" });
                } else {
          ...
          res.status(200).send({
            menssage: "El usuario no puede registrarse",
          });
        }
      }
    });
      ...
      if (user) {
        bcrypt.compare(password, user.password, (err, check) => {
          if (check) {
            ...
            res.status(404).send({
              menssage:
                "El usuario no ha podido loguearse contraseña incorrectamente",
            });
          }
        });
      } else {
        res.status(404).send({
          menssage: "El usuario no ha podido loguearse",
        });
      }
    }
  });
...
function updateUser(req, res) {
  ...
  if (userId != req.user.sub) {
    return res
      .status(500)
      .send({ menssage: "No tiene permitido actualizar el usuario" });
  }
  User.findByIdAndUpdate(userId, update, { new: true }, (err, userUpdate) => {
    if (err) {
      ...
      if (!userUpdate) {
        res.status(404).send({
          menssage: "no se a podido actualizar el usuario",
        });
      } else {
